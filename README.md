...
O KAZER combina uma interface web/PWA com funções serverless e um banco Supabase protegido por RLS. O navegador nunca é a fonte de verdade para autenticação ou autorização: cada operação é validada novamente no servidor.

| Área | Estado atual |
|---|---|
| Continuidade do chat | Mensagens que falham permanecem no histórico e podem ser reenviadas sem perder contexto. |
| Chat | Mensagens que falham permanecem no histórico e podem ser reenviadas sem perder contexto. |
| Limites técnicos | O servidor limita corpo, histórico, caracteres, upstreams, timeouts e frequência por IP/usuário. |
...
O KAZER é uma aplicação estática hospedada na Vercel, com páginas HTML/CSS/JavaScript no diretório `interface/`, funções serverless Node.js no diretório `api/` e autenticação, preferências, avisos e retenção de conta apoiados pelo Supabase. O processamento de chat usa um provedor de modelos configurado no servidor; a pesquisa do WebKazer utiliza provedores públicos de busca e um serviço de resumo configurado no servidor.
...
O fluxo principal segue quatro etapas. Primeiro, a pessoa cria uma conta ou entra pelo Supabase Auth. Depois, a tela `interface/chat.html` recupera a sessão, carrega o perfil e as preferências. Ao enviar uma mensagem, o navegador envia somente o histórico textual permitido para `POST /api/chat`, sempre com autenticação. A API valida origem, sessão, tamanho, conteúdo, moderação e limites técnicos antes de chamar o provedor de IA. Por fim, a resposta é sanitizada e renderizada na interface.
...
flowchart LR
    ...
    V --> G[Provedor de IA do servidor]
...
| Workspace | Painel fechado por padrão, aberto pelo raio, com tarefas, progresso, resultados e repositórios. | `interface/chat.html`, `interface/kazer-workspace.js`, `api/tasks.js` e `kazer_tasks`. |

## Arquitetura

A aplicação é deliberadamente dividida entre um cliente estático e funções serverless. O navegador pode conter somente a URL do Supabase e a chave pública `anon`/publishable; chaves de provedores de IA, `service_role` e segredo do cron devem ficar nas variáveis privadas da Vercel. O cliente nunca deve ser tratado como fonte de verdade para identidade ou autorização.
...
| PostgreSQL/Supabase | Perfis, preferências, avisos, memórias, conectores, tarefas e conversas. | RLS, `FORCE ROW LEVEL SECURITY`, grants mínimos e isolamento por usuário. |
...
| `api/` | Handlers serverless: chat, busca, retenção, MCPs, tarefas, GitHub e módulos de segurança. |
...
| 5 | `009_google_drive_connections.sql` | Tokens cifrados e RLS para Google Drive. |
| 6 | `010_mcp_github_tasks.sql` | MCPs, conexão GitHub e tarefas do workspace sem metadados de cobrança. |
...
| 10 | `015_direct_conversations.sql` | Conversas diretas com RLS por participante. |
| 11 | `017_notification_retention.sql` | Confirmações globais e limpeza de avisos antigos. |
| 12 | `018_remove_consumption_controls.sql` | Remove estruturas legadas de planos, saldo, consumo e anexos. |

Depois das migrações, valide com duas contas de teste que cada usuário só consegue ler ou alterar seu próprio perfil, preferências, memórias, conectores, tarefas, conversas e notificações. Teste também chamadas sem bearer, bearer inválido, sessão revogada e tentativa de enviar `user_id` de outra conta. Não habilite exclusões de retenção antes de testar backup e restauração.
...
|---|---|---|---|
| `/api/chat` | `POST` | `{ messages, mcpConnectorIds, githubRepo }` | `message`, contexto de repositório e metadados de memória/MCP; `401`, `413`, `422`, `429`, `502` conforme a falha. |
...
O histórico aceito pelo chat tem no máximo 72 itens recebidos, mantém os 24 mais recentes, limita cada mensagem a 8.000 caracteres e o total a 32.000 caracteres. O servidor limita o corpo, não aceita anexos no chat e não executa conteúdo recebido.
...
| Autenticação | Chat, pesquisa, conectores e tarefas validam o bearer token no endpoint Auth do Supabase. |
...
| Prompt injection | Mensagens e resultados web são tratados como dados não confiáveis no prompt server-side. |
...
| Conteúdo do chat | Mensagens textuais passam por autenticação, moderação, limites de tamanho, limites de histórico e escaping no cliente. |
...
Os limites em memória são best effort em ambiente serverless e não substituem rate limiting distribuído ou controles operacionais do provedor. A proteção contra prompt injection baseada em instruções não garante que todo ataque seja eliminado. A revisão humana, a configuração correta do Supabase/Vercel e testes autorizados continuam necessários.

## Privacidade e retenção

O KAZER pode tratar dados de cadastro, mensagens, preferências e informações técnicas necessárias à autenticação, segurança, operação e melhoria do serviço. Integrações externas, como Google Drive, recebem somente o conteúdo necessário à ação autorizada. Quando uma função usa serviço externo, o conteúdo necessário pode ser transmitido a esse serviço. O usuário deve evitar inserir senhas, tokens, dados bancários completos, documentos de terceiros ou informações sensíveis que não sejam indispensáveis.
...
| Caracteres | Até 8.000 caracteres por mensagem e 32.000 no histórico processado. |
| Corpo | O chat rejeita requisições que excedam 7 MB; a pesquisa e as integrações possuem limites próprios. |
| Resposta | Até 16.000 caracteres após limpeza do conteúdo do modelo. |
...
| MCPs | Presets e servidores personalizados são gerenciados no Perfil; ferramentas só são chamadas quando conectadas e autorizadas. |
...
O projeto está em desenvolvimento. O fluxo principal de autenticação, chat textual, pesquisa, PWA, retenção, MCPs, GitHub e workspace está implementado no código desta branch. A publicação exige aplicar as migrações ativas e configurar as variáveis privadas descritas acima. A matriz detalhada e as pendências operacionais estão em [`SECURITY.md`](SECURITY.md).
...
O KAZER agora possui um plugin Google Drive com autorização OAuth 2.0 pelo servidor. A pessoa é redirecionada para a tela oficial do Google; o KAZER não coleta senha. O plugin permite buscar/listar arquivos, ler o conteúdo de um arquivo por ID e criar/upload de arquivos de até 5 MB. As ações são autorizadas pela sessão da própria conta e não dependem de saldo ou RPC de consumo.
